...
void JoyCmdBridge_Update(void) {
    ...
    if (!Safety_IsEmergency()) {
        ...
        /* Hysteresis: only recompute target if knob moved enough.
         * Uses combined X+Y delta magnitude to catch diagonal tremors too. */
        int dx   = (int)knob_x - (int)_last_raw_x;
        int dy   = (int)knob_y - (int)_last_raw_y;
        int dist = (int)sqrtf((float)(dx * dx + dy * dy));

        if (dist > JOY_CMD_HYSTERESIS) {
            _last_raw_x     = knob_x;
            _last_raw_y     = knob_y;
            _target_voltage = _KnobToVoltage_Angle(knob_x, knob_y);
        }

        /* Slew: smoothly ramp output toward target every tick */

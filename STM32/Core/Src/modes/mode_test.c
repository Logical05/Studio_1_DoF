...
void Mode_Test_Precision(void) {
    ...
    if (!started) {
        ...
        Trajectory_Start(test_points, repeat * 2U, QEI_GetTheta(), TRAJ_VMAX,
                         TRAJ_AMAX);
...
void Mode_Test_Performance(void) {
    if (!started) {
        ...
        Trajectory_Start(&target_position, 1U, current_position,
                         charmander.perf_velocity, charmander.perf_acceleration);
...
void Mode_Test_Update(void) {
    switch (charmander.test_type) {
        case CHARMANDER_TEST_PRECISION  : Mode_Test_Precision(); break;
        case CHARMANDER_TEST_PERFORMANCE: Mode_Test_Performance(); break;

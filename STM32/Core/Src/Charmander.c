...
void Charmander_Tick(void) {
	...
	// Timeout check
	if ((now - charmander.hb_last_alive_ms) >= CHARMANDER_HB_TIMEOUT_MS) {
		charmander.heartbeat = CHARMANDER_HB_DEAD;
		charmander.hb_rx_status = CHARMANDER_RX_NO_REPLY;
	} else {
...
void Charmander_BuildReadReply(uint16_t start_addr, uint16_t reg_count) {
	...
	/* Track outgoing heartbeat */
	charmander.hb_last_sent_ms = HAL_GetTick();
	charmander.hb_tx_status = CHARMANDER_TX_SENT;

...
static JoyState_t _state = { .buttons = JOY_NONE, .left = { .x = 0, .y = 0 },
		.right = { .x = 0, .y = 0 }, .btn_up = 0, .btn_down = 0, .btn_left = 0,
		.btn_right = 0, .btn_circle = 0, .btn_triangle = 0, .btn_square = 0,
		.btn_xmark = 0, .btn_select = 0, .btn_start = 0, .btn_l1 = 0, .btn_l2 =
				0, .btn_r1 = 0, .btn_r2 = 0, };

static uint8_t _packet_buf[JOY_PACKET_BUF_SIZE];
static uint8_t _packet_idx = 0;

/* Separate tracking ticks to prevent D-pad and Action buttons from refreshing each other */
...
static void _ProcessButtonByte(uint8_t b) {
	...
	if (btn != JOY_NONE) {
		...
		if (btn == JOY_UP || btn == JOY_DOWN || btn == JOY_LEFT
			...
			/* Action/Face button event received. Accumulate and refresh ITS OWN timer */
			_active_actions |= btn;
...
void JOY_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (_huart == NULL || huart->Instance != _huart->Instance)
		return;
		...
		JoyAxis_t JOY_GetRightAxis(void) {
			return (JoyAxis_t ) { _state.right.x, _state.right.y } ;
				...
				uint8_t JOY_LeftActive(int16_t deadzone) {
					return ((_state.left.x > deadzone
							|| _state.left.x < -deadzone)
							|| (_state.left.y > deadzone
									|| _state.left.y < -deadzone)) ? 1 : 0;
				}

				uint8_t JOY_RightActive(int16_t deadzone) {
					return ((_state.right.x > deadzone
							|| _state.right.x < -deadzone)
							|| (_state.right.y > deadzone
									|| _state.right.y < -deadzone)) ? 1 : 0;
				}
				...
				static void _ParsePacket(void) {
					...
					if (buf[0] == 'W') {
						...
						if (p_ptr) {
							_state.left.x = _Remap((uint8_t) atoi(buf + 1),
									127);
							_state.left.y = _Remap((uint8_t) atoi(p_ptr + 1),
									128);
						}
					} else if (buf[0] == 'Q') {
						char *s_ptr = strchr(buf, 'S');
						if (s_ptr) {
							_state.right.x = _Remap((uint8_t) atoi(buf + 1),
									127);
							_state.right.y = _Remap((uint8_t) atoi(s_ptr + 1),
									128);
						}
					}
				}
				...
				static void _SyncFlags(void) {
					...
					_state.btn_triangle =
							(_state.buttons & JOY_TRIANGLE) ? 1 : 0;
